...
---
# Работа с текстами: `gutenbergr`, `tidytext`, `stopwords` и другие
...
* `udpipe` представляет лемматизацию, морфологический и синтаксический анализ разных языков. Туториал можно найти [здесь](https://bnosac.github.io/udpipe/docs/doc1.html), там же есть список доступных языков.
* `phonTools` инструменты для фонетического и акустического анализа
* `lingtypology` пакет для лингвистического картографирования, API для популярных лингвистических баз данных. Туториал [здесь](https://ropensci.github.io/lingtypology/).
* `phonfieldwork` пакет, который поэтапно помогает в проведение фонетического исследования. Туториал [здесь](https://agricolamz.github.io/phonfieldwork/).